...
let eval_add arg1 arg2 =
  let rec add a b =
	...
	| Cons _ -> raise (Error "add can only be applied to int")
      end
    | Cons _ -> raise (Error "add can only be applied to int")
  in
  Atom (string_of_int (add arg1 arg2))
...
let eval_add2 arg =
  let rec add arg =
    ...
    | Cons (car, cdr) -> (add car) + (add cdr)
  in
  Atom (string_of_int (add arg))
...
let eval_sub arg1 arg2 =
  let rec sub a b =
    match a with
      ...
      begin match b with
	  Atom s2 -> (int_of_string s1) - (int_of_string s2)
	| Cons _ -> raise (Error "sub can only be applied to int")
      end
    | Cons _ -> raise (Error "sub can only be applied to int")
  in
  Atom (string_of_int (sub arg1 arg2))
...
let eval_mul arg1 arg2 =
  let rec mul a b =
    match a with
      ...
      begin match b with
	  Atom s2 -> (int_of_string s1) * (int_of_string s2)
	| Cons _ -> raise (Error "mul can only be applied to int")
      end
    | Cons _ -> raise (Error "mul can only be applied to int")
  in
  Atom (string_of_int (mul arg1 arg2))
...
let eval_leq arg1 arg2 =
  let rec leq a b =
    match a with
      ...
      begin match b with
	  Atom s2 -> (int_of_string s1) <= (int_of_string s2)
	| Cons _ -> raise (Error "leq can only be applied to int")
      end
    | Cons _ -> raise (Error "leq can only be applied to int")
  in
  if leq arg1 arg2
...
let eval_geq arg1 arg2 =
  let rec geq a b =
    match a with
      ...
      begin match b with
	  Atom s2 -> (int_of_string s1) <= (int_of_string s2)
	| Cons _ -> raise (Error "geq can only be applied to int")
      end
    | Cons _ -> raise (Error "geq can only be applied to int")
  in
  if geq arg1 arg2
